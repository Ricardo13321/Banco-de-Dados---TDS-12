...
-- 2.1
create view produtos_estoque_baixo as select nome, estoque, preco from produtos where estoque < 15 and ativo = TRUE;
...
create index idx_produtos_clientes_data on pedidos(cliente_id, data_pedido);
create index idx_produtos_categoria_preco on produtos(categoria_id, preco);

-- 5.1
create table importacao_clientes (
	id int auto_increment primary key,
...
load data infile 'C:/Users/Aluno/Downloads/importacao_clientes.csv'
into table importacao_clientes
...
select * from importacao_clientes;

drop table importacao_clientes;

select nome, preco, estoque, categoria_id
from produtos
into outfile 'C:\Users\Aluno\Downloads\produtos_export.csv'
fields terminated by ','
enclosed by '"'
